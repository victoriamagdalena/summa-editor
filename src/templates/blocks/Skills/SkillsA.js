...
const userSkills = (data) => {
  let workSkills = _.chain(_.get(data, "summa['@graph'][1].hasOccupation", []))
    .map("skills")
    .flatten();

  let awardSkills = _.chain(_.get(data, "summa['@graph'][0].award", []))
    .map("skill:assesses")
    .flatten();

  let educationSkills = _.chain(
    _.get(data, "summa['@graph'][1].hasCredential", [])
  )
    .map("teaches")
    .flatten();

  let coursesSkills = _.chain(
    _.get(data, "summa['@graph'][1].hasCredential", [])
  )
    .map("about")
    .flatten()
  ...
  let educationProjectSkills = _.chain(
    _.get(data, "summa['@graph'][1].hasCredential", [])
  )
    .map("about")
    .map("workExample")
  ...
  let interactionTeachSkills = _.chain(
  ...
  let interactionAssessSkills = _.chain(
    _.get(data, "summa['@graph'][1].interactionStatistic", [])
  )
    .map("result")
    .flatten()
  ...
  let allSkills = [
    ...
    ...educationSkills,
    ...coursesSkills,
    ...educationProjectSkills,
    ...interactionTeachSkills,

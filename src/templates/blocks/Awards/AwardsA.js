    ...
    <div className="flex justify-between items-center">
      <div className="flex flex-col text-left mr-2">
        ...
        <span className="text-xs">
          {_.get(item, "['skill:conferredBy'].name", "")}
        </span>
      </div>
      {_.get(item, "['skill:awardedDate']", "") !== "" && (
        <h6 className="text-xs font-medium text-right">
          {formatDate({
            date: _.get(item, "['skill:awardedDate']", ""),
            language,
          })}
        </h6>
      )}
...
const AwardsA = () => {
  ...
  return data.awards && data.awards.enable ? (
    <div>
      <Heading>{data.awards.heading}</Heading>
      <div className="grid gap-4">
        {_.get(data.summa["@graph"][0], "award", [])
          .filter((x) => x["skill:title"] !== "")
          .map((x) => (
            <AwardItem

...
const WorkResponsibilityItem = (x) =>
  x && (
    <li className="mt-2 text-sm" key={uuidv4()}>
      {x}
    </li>
  );
...
const WorkItem = ({ item, language = "en" }) => {
  return (
    <div>
      <div className="flex justify-between items-center">
        <div className="flex flex-col text-left mr-2">
          <h6 className="font-semibold text-sm">
            {_.get(item, "subjectOf.organizer.name", "")}
          </h6>
      ...
      <WorkResponsibility
        responsibilities={_.get(item, "hasOccupation.responsibilities", [])}
      />
...
const WorkA = () => {
  ...
  return _.get(data, "summa['@graph'][1].hasOccupation", []).length > 0 &&
    data.work.enable ? (
    <div>
      <Heading>{data.work.heading}</Heading>
      <div className="grid gap-4">
        {_.get(data, "summa['@graph'][1].hasOccupation", [])
          .filter((x) => !_.get(x, "@id", "").endsWith("disable"))
          .map((x) => (
            <WorkItem

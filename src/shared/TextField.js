...
export default class TextField extends React.Component {
  ...
  initAllValues = (lang) => {
    ...
    let currrentValueIndex = allValues.findIndex(
      (x) => x["@language"] === lang
    );
    ...
    <div key={"holder_" + index} style={{ display: "flex" }}>
      ...
      {_.size(this.props.value) <= 1 ? (
        ...
        <button
          type="button"
          onClick={() => {
            _.pullAt(this.props.value, index);
        ...
        >
          <div className="flex items-center" key={"removeHolder_" + index}>
            <i
              className="material-icons font-bold text-base"
              key={"remove_" + index}
            >
              remove
            </i>
              ...
              value={
                this.props.value[
                  this.props.value.findIndex(
                    (x) => x["@language"] === this.state.editingLanguage
                  ) >= 0
                    ? this.props.value.findIndex(
                        (x) => x["@language"] === this.state.editingLanguage
                      )

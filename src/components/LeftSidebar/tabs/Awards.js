...
const AwardsTab = ({ data, onChange }) => {
  ...
  return (
    <>
      ...
      {_.get(data.summa["@graph"][0], "award", []).map((x, index) => (
        <Item
          item={x}
          key={_.get(x, "@id", "main")}
          index={index}
          ...
          first={index === 0}
          last={
            index === _.size(_.get(data.summa["@graph"][0], "award", [])) - 1
          }
        />
      ))}
  ...
  return (
    <div>
      <TextField
        ...
        onChange={(v) => {
          onChange(`${identifier}['skill:title']`, v);
          if (altidentifier !== "") {
            onChange(`${altidentifier}`, v);
          }
        }}
      />
...
const AddItem = ({ heading, dispatch }) => {
  ...
  const onSubmit = () => {
    ...
    addItem(dispatch, 'data.summa["@graph"][0]["award"]', item);
    addItem(
      dispatch,
      'data.summa["@graph"][1]["award"]',
      _.get(item, "skill:title", "")
    );
...
const Item = ({ item, index, onChange, dispatch, first, last }) => {
  const [isOpen, setOpen] = useState(false);
  const identifier = `data.summa["@graph"][0].award[${index}].`;
  const altidentifier = `data.summa["@graph"][1].award[${index}]`;

    ...
    <>
      <div className="mb-6 grid grid-cols-6 items-center">
        <div className="col-span-1">
          <Checkbox
            checked={data.work.enable}
            onChange={(v) => onChange("data.work.enable", v)}
          />
        </div>
      ...
      {_.get(data.summa["@graph"][1], "hasOccupation", []).map((x, index) => (
        <Item
          dispatch={dispatch}
          first={index === 0}
          index={index}
          item={x}
          key={_.get(x, "@id", "item")}
          last={index === _.size(data.summa["@graph"][1].hasOccupation) - 1}
          onChange={onChange}
        />
      ))}
...
const AddItem = ({ heading, dispatch }) => {
  ...
  const onSubmit = () => {
    ...
    addItem(dispatch, "data.summa['@graph'][1].hasOccupation", item);
...
const Item = ({ item, index, onChange, dispatch, first, last }) => {
  const [isOpen, setOpen] = useState(false);
  const identifier = `data.summa["@graph"][1].hasOccupation[${index}].`;

  return (
    <div className="my-4 border border-gray-200 rounded p-5">
      <ItemHeading
        title={item.roleName + " " + item.subjectOf.organizer.name}
        setOpen={setOpen}
        isOpen={isOpen}
      />
      ...
      <div className={`mt-6 ${isOpen ? "block" : "hidden"}`}>
        ...
        <ItemActions
          ...
          last={last}
          onChange={onChange}
          type="data.summa['@graph'][1].hasOccupation"

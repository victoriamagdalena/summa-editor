...
const EducationTab = ({ data, onChange }) => {
  ...
  return (
    <>
      ...
      {_.get(data.summa["@graph"][1], "hasCredential", []).map((x, index) => (
        <Item
          item={x}
          key={_.get(x, "@id", "item")}
          index={index}
          ...
          last={
            index ===
            _.size(_.get(data.summa["@graph"][1], "hasCredential", [])) - 1
          }
        />
      ))}
  ...
  return (
    <div>
      ...
      <TextField
        ...
        onChange={(v) =>
          onChange(`${identifier}about.educationalCredentialAwarded`, v)
        }
      />
...
const AddItem = ({ heading, dispatch }) => {
  ...
  const onSubmit = () => {
    ...
    addItem(dispatch, 'data.summa["@graph"][1].hasCredential', item);
...
const Item = ({ item, index, onChange, dispatch, first, last }) => {
  const [isOpen, setOpen] = useState(false);
  const identifier = `data.summa['@graph'][1].hasCredential[${index}].`;

  return (
    <div className="my-4 border border-gray-200 rounded p-5">
      <ItemHeading
        title={_.get(item, "about.provider.name", "")}
        setOpen={setOpen}
        isOpen={isOpen}
      />
      ...
      <div className={`mt-6 ${isOpen ? "block" : "hidden"}`}>
        ...
        <ItemActions
          ...
          last={last}
          onChange={onChange}
          type="data.summa['@graph'][1].hasCredential"
